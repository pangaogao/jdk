...
/**
 * 每一个线程都有一个优先级，优先级值越大，执行的优先级越低，每一个线程可能被标记为守护线程
 * 运行的线程创建新的线程，新线程会有创建它线程相同的优先级
 * 守护线程创建的线程还是守护线程
 * <p>
 * 当一个JAVA虚拟机启动，通常伴随着非守护线程（叫做main方法） JAVA虚拟机继续执行直到下面的情况出现：
 * 1.exit方法被调用，安全管理器允许退出操作执行
 * 2.非守护线程已经死亡，或run方法成功返回，或run方法抛出异常
 * <p>
 * 有两种方法来创建新线程执行
 * 1.声明一个集成Thread类的子类，子类重写run方法，子类被分配并启动，例子如下：
 * <p>
 ...
 * new Thread(p).start();
 * <p>
 * 每一个线程有一个名称标识，不止一个线程可能会有相同的名称，如果名称没有指定，会自动生成
 *
 * @author unascribed
 * @see Runnable
...
public class Thread implements Runnable {
     ...
     * @param name                新线程的名称
     * @param stackSize           新线程栈的大小 the desired stack size for the new thread, or
     *                            zero to indicate that this parameter is to be ignored.
     * @param acc                 the AccessControlContext to inherit, or
     *                            AccessController.getContext() if null
     * @param inheritThreadLocals true的时候，继承父线程的本地变量 if {@code true}, inherit initial values for
    ...
    private void init(ThreadGroup g, Runnable target, String name,
        ...
        /** 子线程会根据父线程要继承的本地变量，创建Map副本，所以子线程和父线程的本地变量是隔离的 */
    ...
    public synchronized void start() {
        ...
        /** 提醒这个线程要启动了，以便加入到线程组，线程组中未启动的线程数就可以减少了 */
        group.add(this);
